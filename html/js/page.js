$.fn.extend({
    paging: function (option) {
        ...
        $.ajax({
            async: false,
            url: settings.url + "?page=1&pageSize=5",
            type: "GET",
            success: function (res) {
                settings.totalPage = res.data.pages;
                //初始化页面
                var type = settings.url.toString().split("/")[1];
                var type2 = settings.url.toString().split("/")[2];
                ...
                if (type === "book") {
                    ...
                    if (type2 === "customer") {
                        viewCustomerOrderList(list)
                    }
                    else {
                        viewOrderDetail(list)
                    }
